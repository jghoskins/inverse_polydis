...
while (ifflag == false)
  iloop = iloop + 1;
  if (iloop >=nstp_max && vnorm_rel/vnmx<0.01)
      ier = 0;
      return
  end
  if (iloop >=nstp_max+1)
...
[u_new,u_grad,chnkr_s,grad,bd_sol,bd_data_grad,bd_sol_grad,F_s,err_est] = ...
...
if (e_old >e_new_newt) 
 ...
 inewt = false;
 ifused = true;
elseif (e_old > e_new_newt*0.2 && e_old <= e_new_newt)
  vguess = vguess/2;
  inewted= inewted + 1;
else
   inewted = 8;
